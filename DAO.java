...
public class DAO {
    ...
    public boolean conectar () {
        String driverName = "org.postgreswl.Driver";
   ...
   public boolean inserir Usuario (Usuario usuario) {
       boolean status = false;
       try {
    ...
    public boolean excluirUsuario (int codigo) {
        ...
        public Usuario[] getUsuarios() {
         ...
         public Usuario[] getUsuariosMasculinos() {
             ...
             try {
                 Statement st = conexao.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet,CONCUR_RED_ONLY);
                 ResultSet rs = st.executeQuery("SELECT * FROM usuario WHERE usuario.sexo LIKE 'M'");
                  if(rs.next()) {
                      ...
                      for(int i = 0; rs.next(); i++ {
                             usuarios[i] = new Usuario(rs.getInt("codigo"), rs.getString("login"),
                                                       rs.getString("senha"), rsgetString("sexo").charAt(0));
                      }
                   }
                   st.close();
              } catch (Exception e) {
